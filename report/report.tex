...
\end{tabular}
\caption{Comparison of classifier scores} \label{tab:classificationresults}
\end{center}
\end{table}
\FloatBarrier
...
\hline
Iteration 	& Accuracy  & CV mean & CV std & precision & recall & F1 \\ \hline
...
\end{tabular}
\caption{Classifier scores comparison on each iteration}
...
As can be seen from Table \ref{tab:lin_gen_scores}, accuracy on the gold
test dataset, precision and F1 score are steadily increasing (although
values for 5-fold cross validation (CV) score and recall are not stable.  
From Figure \ref{lsvc_cm}, we can also see that the
