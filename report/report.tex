...
\begin{figure}[h]
	\centering
 	\includegraphics[scale=0.5]{best_w2v_cm.png}
...
The best model is LDA with 400 topics according to scores and with priority to purity and homogeneity ones as \review{it's more important for us to have one intent in one cluster even if some intents will be present in several ones.}
...
Following \add{( et al.)} we trained BiLSTM encoder model experimenting with different hper-parameters such as  the number of layers, the dimension of the word embedding and the type of pooling.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.30]{svc0_cm.png}
...
\add{if you include the above confusion matrix you need to comment on them: what do they show?}
...
\add{The following sentence is unclear. What do you mean by ``provide expansion to all categories'' ? what shows this ? }: BiLSTM model didn't succeded to provide expansion to all categories so result only for LDA model are provided.
